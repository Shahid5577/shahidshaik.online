...
import SwiprrHomeFeature, {swiprrFeatures} from "../components/features/SwiprrFeatures";
import card from './images/Card.png';


interface SwiprrHomeProps {}

const SwiprrHome: React.FC<SwiprrHomeProps> = ({}) => {

  useEffect(() => {
    updateTitleAndFavicon("Clikkle Swiprr | A Dating Experience That Feels Genuinely Wonderful", "https://cdn.clikkle.com/images/swiprr/logo/2023/swiprr.png");
  }, []);
    ...
    <div  className="px-2 py-8 sm:px-6 sm:py-6">
      <div id='home-section' className="p-4 flex flex-col sm:flex-row mt-0 sm:items-center justify-center sm:justify-start h-[calc(68vh-65px)] ml-2 gap-10">
        <div className="max-w-[600px] px-4 sm:px-8 pb-12 sm:mr-10 gap-6 text-center text-sm">
          ...
          >
          Date expressively with assurance on SWIPRR
          </Typography>
          ...
          >
            Adding more of your authentic self to create a dating experience that feels genuinely wonderful.
        </Typography>
          <Typography
        ...
        <div className="items-center justify-center hidden ml-16 sm:flex ">
          <img
            src={card}
            className="h-72 w-auto lg:h-[calc(110vh-300px)] sm:h-[calc(60vh-300px)]"
            alt="Banner"
          />
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center gap-5 sm:mt-7">
        {swiprrFeatures.map((swiprrFeatures, index) => (
            <SwiprrHomeFeature
             {...{ swiprrFeatures, left: index % 2 === 0, key: swiprrFeatures.name }}
          />
        ))}
      </div>
