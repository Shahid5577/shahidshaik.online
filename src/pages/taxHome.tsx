...
import TaxHomeFeature, {taxFeatures} from "../components/features/taxFeatures";

interface TaxHomeProps {}

const TaxHome: React.FC<TaxHomeProps> = ({}) => {
  useEffect(() => {
    updateTitleAndFavicon("Clikkle Tax | #1 Ai Powered Tax Preparation Software ", "https://cdn.clikkle.com/images/tax/logo/2023/tax.png");
  }, []);
    ...
    <div  className="px-2 py-8 sm:px-6 sm:py-6">
      <div id='home-section' className="p-4 flex flex-col sm:flex-row mt-0 sm:items-center justify-center sm:justify-start h-[calc(68vh-65px)] ml-2 gap-10">
        <div className="max-w-[600px] px-4 sm:px-8 pb-12 sm:mr-10 gap-6 text-center text-sm">
          <Typography
            variant="h3"
            className="hidden mt-4 font-light text-center text-gray-800 sm:flex sm:text-left dark:text-gray-300 pr-64"
          >
          ...
          >
          We guarantee the biggest refund by doing all the work
          </Typography>
          <Typography
            className="font-normal text-center text-gray-800 sm:hidden sm:text-left dark:text-gray-300 sm:mt-0"
        ...
        <div className="items-center justify-center hidden ml-16 sm:flex ">
          <img
            src="https://cdn.clikkle.com/images/tax/illustration/Frame%201%20(2).png"
      ...
      <div className="flex flex-col items-center justify-center gap-5 sm:mt-7">
        {taxFeatures.map((taxFeatures, index) => (
            <TaxHomeFeature
             {...{ taxFeatures, left: index % 2 === 0, key: taxFeatures.name }}
          />
        ))}
      </div>
          ...
          <div className="flex items-center justify-center gap-4">
            <Button variant="contained" size="small">
              <Link to="https://clikkle.com/tryclikkleplus">
                Try Clikkle Tax for Free
              </Link>
            </Button>
            <Button variant="outlined" size="small">
              <Link to="https://clikkle.com/gotoplus">Go to Clikkle Tax</Link>
            </Button>
          </div>
        </div>
