...
import AdsHomeFeature, {adsFeatures} from "../components/features/adsFeatures";

interface AdsHomeProps {}

const AdsHome: React.FC<AdsHomeProps> = ({}) => {

  useEffect(() => {
    updateTitleAndFavicon("Clikkle Ads | Get More Customers With Online Advertising", "https://cdn.clikkle.com/images/ads/logo/2023/ads.png");
  }, []);
  ...
  return (
    <div  className="px-2 py-8 sm:px-6 sm:py-6">
      <div id='home-section' className="p-4 flex flex-col sm:flex-row mt-0 sm:items-center justify-center sm:justify-start h-[calc(68vh-65px)] ml-2 gap-10">
        <div className="max-w-[600px] px-4 sm:px-8 pb-12 sm:mr-10 gap-6 text-center text-sm">
          ...
          >
            ...
            platform. Get programmatic buying, retargeting, and
            real-time optimization for maximum ROI. Join now.
        </Typography>
          <Typography
      ...
      <div className="flex flex-col items-center justify-center gap-5 sm:mt-7">
        {adsFeatures.map((adsFeatures, index) => (
            <AdsHomeFeature
             {...{ adsFeatures, left: index % 2 === 0, key: adsFeatures.name }}
          />
        ))}
      </div>
