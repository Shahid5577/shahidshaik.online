...
import ChatHomeFeature, {chatFeatures} from "../components/features/chatFeatures";

interface ChatHomeProps {}

const ChatHome: React.FC<ChatHomeProps> = ({}) => {
  useEffect(() => {
    updateTitleAndFavicon("Clikkle Chat | Making Internet Business Personal", "https://cdn.clikkle.com/images/chat/logo/2023/chat.png");
  }, []);
    ...
    <div  className="px-2 py-8 sm:px-6 sm:py-6">
      <div id='home-section' className="p-4 flex flex-col sm:flex-row mt-0 sm:items-center justify-center sm:justify-start h-[calc(68vh-65px)] ml-2 gap-10">
        <div className="max-w-[600px] px-4 sm:px-8 pb-12 sm:mr-10 gap-6 text-center text-sm">
          ...
          >
            Engage your customers with Clikkle Chat
          </Typography>
          ...
          >
            Clikkle chat swiftly resolves issues, boosting team efficiency while also enhancing customer satisfaction.
           </Typography>
          <Typography
            className="font-normal text-center text-gray-800 sm:hidden sm:text-left dark:text-gray-300 sm:mt-0"
        ...
        <div className="items-center justify-center hidden  ml-16 sm:flex ">
          <img
            src="https://cdn.clikkle.com/images/chat/Clikkle%20Chat%20page/Ellipse%201.png"
            className="h-72 w-auto lg:h-[calc(115vh-300px)] sm:h-[calc(60vh-300px)]"
            alt="Banner"
          />
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center gap-5 sm:mt-7">
        {chatFeatures.map((chatFeatures, index) => (
            <ChatHomeFeature
             {...{ chatFeatures, left: index % 2 === 0, key: chatFeatures.name }}
          />
        ))}
      </div>
          ...
          <div className="flex items-center justify-center gap-4">
            <Button variant="contained" size="small">
              <Link to="https://clikkle.com/tryclikkleplus">
                Try Clikkle Chat for Free
              </Link>
            </Button>
            <Button variant="outlined" size="small">
              <Link to="https://clikkle.com/gotoplus">Go to Clikkle Chat</Link>
            </Button>
          </div>
        </div>
